...
function checkExpoAlignment() {
  ...
  if (result.error) {
    fail(`Could not run \`npx expo install --check\`: ${result.error.message}`);
    return false;
  }
  if (result.status === 0) {
...
function checkOutdated() {
  ...
  if (result.error) {
    warn(`Could not run \`npm outdated\`: ${result.error.message}`);
    return;
  }
...
function checkAudit() {
  ...
  if (result.error) {
    warn(`Could not run \`npm audit\`: ${result.error.message}`);
    warn('Vulnerability check inconclusive — re-run `npm audit` manually.');
    return;
  }
  ...
  if (parsed.error) {
    warn(`npm audit failed: ${parsed.error.summary || parsed.error.code || 'unknown error'}`);
    warn('Vulnerability check inconclusive — re-run `npm audit` manually.');
    return;
  }

  if (!parsed.metadata || !parsed.metadata.vulnerabilities) {
    warn('npm audit returned an unexpected shape. Run `npm audit` manually.');
    return;
  }
...
function runFix() {
  ...
  if (!checkExpoAlignment()) {
    warn('Some Expo packages still mismatched after fix — likely peer-dep conflicts. Resolve manually.');
    process.exit(1);
  }
